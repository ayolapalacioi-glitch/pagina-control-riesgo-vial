(function attachMapAdapter(globalScope) {
  class MapAdapter {
    ...
    toLatLng(point, canvasSize) {
      const nPoint = this.normalizePoint(point, canvasSize);
      const lat = this.bounds.north - (this.bounds.north - this.bounds.south) * nPoint.y;
      const lng = this.bounds.west + (this.bounds.east - this.bounds.west) * nPoint.x;
      return { lat, lng };
    }
